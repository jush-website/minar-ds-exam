...
// --- 通用工具函數：插入格式 ---
const performInsertFormat = (el, type, currentVal, onUpdate) => {
  if (!el) return;
  ...
  if (type === 'latex') { newText = currentVal.substring(0, start) + '$' + selectedText + '$' + currentVal.substring(end); offset = 1; }
  else if (type === 'code') { newText = currentVal.substring(0, start) + '\n```\n' + selectedText + '\n```\n' + currentVal.substring(end); offset = 5; }
  else if (type === 'sup') { newText = currentVal.substring(0, start) + '^{' + selectedText + '}' + currentVal.substring(end); offset = 2; }
  else if (type === 'sub') { newText = currentVal.substring(0, start) + '_{' + selectedText + '}' + currentVal.substring(end); offset = 2; }
...
const EditorToolbar = ({ onInsert }) => (
  <div className="flex flex-wrap gap-2 mb-2">
    <button type="button" onClick={() => onInsert('latex')} className="flex items-center gap-1 px-2 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg text-[0.6rem] font-black border border-indigo-100 hover:bg-indigo-100 transition-colors"><Sigma size={12} /> 公式</button>
    <button type="button" onClick={() => onInsert('sup')} className="flex items-center gap-1 px-2 py-1.5 bg-blue-50 text-blue-600 rounded-lg text-[0.6rem] font-black border border-blue-100 hover:bg-blue-100 transition-colors"><Superscript size={12} /> 上標</button>
    <button type="button" onClick={() => onInsert('sub')} className="flex items-center gap-1 px-2 py-1.5 bg-cyan-50 text-cyan-600 rounded-lg text-[0.6rem] font-black border border-cyan-100 hover:bg-cyan-100 transition-colors"><Subscript size={12} /> 下標</button>
    <button type="button" onClick={() => onInsert('code')} className="flex items-center gap-1 px-2 py-1.5 bg-slate-100 text-slate-600 rounded-lg text-[0.6rem] font-black border border-slate-200 hover:bg-slate-200 transition-colors"><Terminal size={12} /> 程式碼</button>
  </div>
);
  ...
  <div className="min-h-screen flex flex-col items-center justify-center bg-slate-900 p-4 text-white font-sans text-center">
    <div className="bg-white/5 p-10 rounded-[3rem] backdrop-blur-xl border border-white/10 flex flex-col items-center shadow-2xl max-w-sm w-full">
      ...
      <div className="grid grid-cols-1 gap-4 w-full">
        ...
        >
          <ShieldCheck size={24} />
          <span className="font-bold text-white text-white">助教後台管理</span>
        </button>
      </div>
    </div>
  </div>
...
const StudentLoginView = memo(({ studentInfo, setStudentInfo, onStartExam, onBack, isChecking }) => (
  <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4 font-sans text-slate-800 text-center">
    <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl w-full max-w-md border border-slate-100 flex flex-col items-center text-center text-slate-800">
      <h2 className="text-2xl font-black mb-2">身份核對</h2>
...
const StudentExamView = memo(({ questions, studentInfo, currentAnswers, setCurrentAnswers, isTestMode, onSubmit, onCancel }) => {
  ...
  const handleFillInsert = (qId, type) => {
    const el = inputRefs.current[qId];
    const currentVal = currentAnswers[qId] || "";
    performInsertFormat(el, type, currentVal, (newVal) => {
      setCurrentAnswers(prev => ({ ...prev, [qId]: newVal }));
    });
  };
    ...
    <div className="min-h-screen bg-slate-50 select-none pb-20 text-left font-sans text-slate-800">
      <header className={`border-b sticky top-0 z-30 p-4 flex justify-between items-center px-8 shadow-sm ${isTestMode ? 'bg-amber-500 text-white' : 'bg-white text-slate-800'}`}>
        <div className="font-black tracking-tight flex items-center gap-2 uppercase text-sm">
          {isTestMode ? <PlayCircle size={18} /> : <div className="w-2.5 h-2.5 bg-red-500 rounded-full animate-ping text-white"></div>}
      ...
      <main className="max-w-3xl mx-auto p-4 py-8 space-y-8 text-left text-slate-800">
        {!isTestMode && (
          ...
          <div key={q.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden text-left text-slate-800">
            ...
            {(q.type === 'choice' || q.type === 'multiple') ? (
              ...
              <div className="space-y-2">
                <EditorToolbar onInsert={(type) => handleFillInsert(q.id, type)} />
                <textarea 
                  ref={el => inputRefs.current[q.id] = el}
                  className="w-full p-5 rounded-2xl bg-slate-50 border-2 border-transparent focus:border-indigo-600 focus:bg-white outline-none transition-all font-bold text-slate-800 shadow-inner min-h-[120px] font-mono text-base" 
                  placeholder="請在此輸入您的回答... 可使用上方工具輔助格式" 
...
const ResultView = memo(({ examResult, isTestMode, onBack }) => (
  <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4 font-sans text-slate-800 text-center text-slate-800">
    <div className="bg-white p-10 rounded-[3rem] shadow-2xl w-full max-lg text-center border border-slate-100 flex flex-col items-center">
      <div className={`w-24 h-24 rounded-full flex items-center justify-center mb-8 shadow-inner ${examResult?.isTerminated ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
      ...
      <div className={`w-full bg-gradient-to-br p-10 rounded-[2.5rem] mb-10 shadow-xl text-white ${isTestMode ? 'from-amber-500 to-amber-700' : (examResult?.isTerminated ? 'from-red-500 to-red-700' : 'from-slate-800 to-slate-900')}`}>
        <span className="text-[0.7rem] font-black uppercase tracking-[0.3em] opacity-60 mb-2 block text-center text-white">Choice Score (選擇得分)</span>
...
const AdminEditQuestionModal = memo(({ editingQ, setEditingQ, saveQuestion, showToast }) => {
    ...
    for (let index in items) {
      ...
      if (item.kind === 'file' && item.type.startsWith('image/')) {
        ...
        reader.onload = (event) => {
          ...
          const start = el.selectionStart;
          const currentVal = optionIdx !== -1 ? localQ.options[optionIdx] : localQ[field];
          
          performInsertFormat(el, 'image', currentVal, (newVal) => {
             // 這裡手動拼裝，因為 performInsertFormat 沒寫圖片邏輯
             const textBefore = currentVal.substring(0, el.selectionStart);
             const textAfter = currentVal.substring(el.selectionEnd);
             const fullNew = textBefore + `\n![image](${base64})\n` + textAfter;
             if (optionIdx !== -1) handleOptionUpdate(optionIdx, fullNew);
             else handleUpdate(field, fullNew);
          });
  ...
  const onInsertFormat = (targetRef, type, field, optionIdx = -1) => {
    const el = targetRef.current;
    const currentVal = optionIdx !== -1 ? localQ.options[optionIdx] : localQ[field];
    performInsertFormat(el, type, currentVal, (newVal) => {
      if (optionIdx !== -1) handleOptionUpdate(optionIdx, newVal);
      else handleUpdate(field, newVal);
    });
  };
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4 font-sans text-slate-800 text-left">
      <div className="bg-white rounded-[3rem] w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl border border-white/20 text-left">
        <div className="p-6 border-b flex justify-between items-center bg-slate-50 text-slate-800">
          <div className="flex items-center gap-3 text-slate-800"><div className="bg-indigo-600 p-2 rounded-xl text-white text-center"><Settings size={20}/></div><h3 className="text-xl font-black">考題編輯</h3></div>
          <button onClick={() => setEditingQ(null)} className="p-2 hover:text-red-500 transition-colors text-slate-400 flex items-center justify-center text-slate-400 text-white"><X size={32}/></button>
        </div>
        <div className="flex flex-col lg:flex-row overflow-hidden flex-1">
          <form onSubmit={(e) => { e.preventDefault(); saveQuestion(new FormData(e.target)); }} className="p-8 space-y-6 overflow-y-auto lg:w-1/2 border-r border-slate-100 text-left text-slate-800">
            <div>
              <label className="block text-[0.65rem] font-black text-slate-400 mb-2 uppercase ml-1">題目內容</label>
              <EditorToolbar onInsert={(type) => onInsertFormat(questionRef, type, 'text')} />
            ...
            <div className="grid grid-cols-2 gap-4 text-slate-800">
              ...
              <select name="type" value={localQ.type} onChange={(e) => handleUpdate('type', e.target.value)} className="w-full p-4 bg-slate-50 rounded-2xl font-bold border-none outline-none shadow-inner text-slate-700 appearance-none cursor-pointer text-slate-800"><option value="choice">單選題</option><option value="multiple">複選題</option><option value="fill">填空題</option></select></div>
                ...
                {['A', 'B', 'C', 'D'].map((lab, i) => (
                  <div key={lab} className="flex gap-2 items-start text-slate-800">
                    ...
                    <div className="flex-1 text-left">
                      <EditorToolbar onInsert={(type) => onInsertFormat(optionRefs[i], type, `opt${lab}`, i)} />
            ...
            <div className="flex gap-4 pt-4 text-white text-center">
          ...
          <div className="p-8 lg:w-1/2 bg-slate-50/50 overflow-y-auto flex flex-col text-left text-slate-800">
            ...
            <div className="space-y-6 flex-1 text-slate-800">
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 text-slate-800 text-left"><FormattedText content={localQ.text || "等待輸入..."} className="text-base" /></div>
              {(localQ.type === 'choice' || localQ.type === 'multiple') && (
                <div className="space-y-3 text-left">{localQ.options.map((opt, i) => (
                  <div key={i} className="bg-white p-4 rounded-2xl border border-slate-100 flex items-start gap-4 text-left">
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-800 text-left text-slate-800">
      ...
      <div className="flex-1 max-w-6xl w-full mx-auto p-6 md:p-10 flex flex-col text-slate-800 text-left">
        ...
        {tab === 'records' && (
          <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden text-left">
            ...
            <div className="overflow-x-auto text-left"><table className="w-full text-left border-collapse"><thead className="bg-slate-50 text-[0.6rem] font-black text-slate-400 uppercase tracking-widest text-left"><tr><th className="p-6">姓名 / 學號</th><th>選擇分</th><th>填空分</th><th className="text-center">總分</th><th className="text-center">操作</th></tr></thead>
              <tbody className="divide-y divide-slate-100 text-left text-slate-800">{records.sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp)).map(r => (
                <tr key={r.id} className="hover:bg-slate-50 transition text-left text-slate-800 text-left"><td className="p-6 font-black">{r.studentName}<div className="font-mono text-[0.6rem] text-indigo-500">{r.studentId}</div></td>
                <td className="font-bold text-slate-500">{r.choiceScore ?? r.score}</td><td className="font-bold text-indigo-600">{r.fillScore ?? 0}</td><td className="text-center"><span className={`px-4 py-1 rounded-full font-black ${ (r.totalScore ?? r.score) >= 60 ? 'text-green-600 bg-green-50' : 'text-red-600 bg-red-50' } text-center`}>{r.totalScore ?? r.score}</span></td>
        ...
        {tab === 'exams' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left text-slate-800 text-left">
            {exams.map(e => (
              <div key={e.id} className={`bg-white p-8 rounded-[2.5rem] border-2 flex flex-col justify-between ${e.isActive ? 'border-green-500 shadow-xl' : 'border-slate-100'} text-left`}>
                <div className="text-left text-slate-800 text-left"><div className="flex justify-between mb-4 text-left"><span className={`px-3 py-1 rounded-full text-[0.6rem] font-black ${e.isActive ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-400'} text-left text-center`}>{e.isActive ? '開放中' : '關閉中'}</span>
                <div className="flex gap-2 text-slate-300 text-left"><button onClick={() => setEditingExam(e)} className="text-slate-300"><Edit3 size={18}/></button><button onClick={async () => { if(confirm('確定刪除？')) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'exams', e.id)); }} className="text-slate-300"><Trash2 size={18}/></button></div></div>
                <h4 className="text-2xl font-black mb-6">{e.title}</h4></div>
                <div className="flex gap-3 pt-4 border-t text-white text-left text-left"><button onClick={async () => { const all = await getDocs(collection(db, 'artifacts', appId, 'public', 'data', 'exams')); for(let d of all.docs) await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'exams', d.id), { isActive: false }); if(!e.isActive) await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', 'exams', e.id), { isActive: true }); showToast(e.isActive ? "已停止" : "已啟動"); }} className={`flex-1 py-3 rounded-2xl font-black text-xs ${e.isActive ? 'bg-slate-100 text-slate-600' : 'bg-green-600 text-white'} text-white text-center`}>{e.isActive ? '停止測驗' : '啟動正式測驗'}</button>
                <button onClick={() => onTestExam(e)} className="px-6 py-3 rounded-2xl bg-slate-900 text-white font-black text-xs flex items-center gap-2 text-white text-center text-white"><PlayCircle size={16}/> 測試</button></div>
              </div>
            ))}<button onClick={() => setEditingExam({ id: 'new', title: '' })} className="min-h-[200px] border-2 border-dashed border-slate-200 rounded-[2.5rem] flex flex-col items-center justify-center gap-4 text-slate-400 hover:border-indigo-500 hover:text-indigo-500 transition text-slate-400 text-center"><Plus size={48}/><span className="font-black">建立新考卷</span></button>
          </div>
        )}

        {tab === 'questions' && (
          <div className="space-y-6 text-left text-left">
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-200 text-left text-left">
              <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto text-left">
                ...
                {selectedExamId && (
                  <button 
                    ...
                    className="flex items-center gap-2 px-6 py-4 bg-slate-100 text-slate-600 rounded-2xl font-bold hover:bg-slate-200 transition-all text-sm whitespace-nowrap text-center text-slate-600"
                  >
              ...
              {selectedExamId && <button onClick={() => setEditingQ({ id: 'new', type: 'choice', text: '', answer: 'A', points: 4, options: ['', '', '', ''] })} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl flex items-center gap-2 font-black transition text-white text-white text-center text-white">新增題目</button>}
            </div>

            <div className="grid grid-cols-1 gap-6 text-left text-slate-800 text-left">
              {selectedExamId && sortedQuestions.map((q, idx) => (
                <div key={q.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200 flex flex-col md:flex-row gap-6 items-start text-left text-slate-800 text-left">
                  <div className="flex-1 w-full text-left text-slate-800 text-left">
                    <div className="flex flex-wrap items-center gap-3 mb-4 text-left">
                      ...
                      <span className="bg-slate-100 text-slate-400 px-3 py-1 rounded-xl text-xs font-black text-slate-400 text-center">{q.points} Pts</span>
                      {q.timestamp && (
                    ...
                    <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 text-slate-700 text-left text-left"><FormattedText content={q.text} className="text-sm" /></div>
                    <div className="mt-4 font-black text-green-600 px-2 text-sm text-left">標竿答案：{q.answer}</div>
                  </div>
                  <div className="flex md:flex-col gap-2 shrink-0 text-left text-slate-800 text-center text-slate-800"><button onClick={() => setEditingQ(q)} className="p-4 bg-slate-50 text-indigo-500 rounded-2xl shadow-sm text-indigo-500 text-center"><Edit3 size={20}/></button>
                  <button onClick={async () => { if(confirm('確定刪除題目？')) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'questions', q.id)); }} className="p-4 bg-slate-50 text-red-500 rounded-2xl shadow-sm text-red-500 text-center"><Trash2 size={20}/></button></div>
                </div>
              ))}
            </div>
          </div>
      ...
      {viewingRecord && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col text-slate-800 text-left">
            <div className="p-8 border-b flex justify-between items-center bg-slate-50 text-slate-800 text-left">
              <div className="flex items-center gap-6 text-slate-800 text-left"><div className="w-16 h-16 bg-slate-900 rounded-3xl flex flex-col items-center justify-center text-white shadow-lg text-white text-center"><span className="text-[0.5rem] font-bold opacity-50 uppercase">Score</span><span className="text-2xl font-black">{(viewingRecord.choiceScore || 0) + gradingAnswers.reduce((acc, curr) => acc + curr.val, 0)}</span></div>
            ...
            <div className="p-10 space-y-8 overflow-y-auto flex-1 bg-slate-50/50 text-left text-slate-800 text-left">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left text-slate-800">{viewingRecord.answers.map((a, idx) => (<div key={idx} className={`p-8 rounded-[2.5rem] border-2 bg-white ${a.type === 'fill' ? 'border-indigo-500 ring-8 ring-indigo-500/5' : 'border-slate-100 opacity-60'} text-left text-slate-800 text-left`}>
                <div className="flex justify-between items-center mb-6 text-xs font-black text-slate-400 uppercase tracking-tighter text-slate-400 text-left"><span>Q{idx+1} • {a.type === 'choice' ? '單選' : a.type === 'multiple' ? '複選' : '填空'}</span><span>{a.points} Pts</span></div>
                ...
                <div className="space-y-4 text-left text-left"><div className="p-4 bg-indigo-50 rounded-2xl text-indigo-900 text-left"><label className="text-[0.6rem] font-black text-indigo-400 uppercase block mb-1 text-left">考生回答</label><FormattedText content={a.studentAns || "(未答)"} className="font-black text-lg text-left" /></div>
            ...
            <div className="p-8 border-t bg-white flex gap-4 justify-center text-white text-white text-center"><button onClick={saveManualGrades} className="px-12 py-5 bg-indigo-600 rounded-[2rem] font-black shadow-xl text-white text-white text-center">儲存分數</button><button onClick={() => setViewingRecord(null)} className="px-12 py-5 bg-slate-100 text-slate-600 rounded-[2rem] font-black text-slate-600 text-center">取消</button></div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4 text-slate-800">
          <div className="bg-white rounded-[2.5rem] p-10 w-full max-w-md shadow-2xl flex flex-col items-center text-slate-800 text-center">
            ...
            <form onSubmit={saveExam} className="space-y-6 w-full text-left text-slate-800">
              ...
              <div className="flex gap-4 justify-center text-white text-white text-center text-white"><button type="submit" disabled={isSaving} className="flex-1 bg-slate-900 py-4 rounded-xl font-black text-white text-white text-center">{isSaving ? '儲存中...' : '確認'}</button><button type="button" onClick={() => setEditingExam(null)} className="flex-1 bg-slate-100 py-4 rounded-xl font-black text-slate-600 text-slate-600 text-center">取消</button></div>
            </form>
          </div>
        </div>
      )}
