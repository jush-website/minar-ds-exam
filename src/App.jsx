...
import { 
  ...
  AlertTriangle, PlayCircle, BookOpen, Layers, X, Check, FileDown, ArrowLeft, Save, Info, CheckCircle, Sigma, Binary
} from 'lucide-react';
...
const LandingView = memo(({ onStart, activeExam, onAdminLogin }) => (
  <div className="min-h-screen flex flex-col items-center justify-center bg-slate-900 p-4 text-white font-sans text-center">
    <div className="bg-white/5 p-10 rounded-[3rem] backdrop-blur-xl border border-white/10 flex flex-col items-center shadow-2xl max-w-sm w-full">
      ...
      <h1 className="text-3xl font-black mb-2 tracking-tighter">資料結構測驗系統</h1>
...
const AdminEditQuestionModal = memo(({ editingQ, setEditingQ, saveQuestion }) => {
  ...
  const toggleFormula = (currentText, isFormula, callback) => {
    if (isFormula) {
      if (!currentText.includes('$$')) {
        callback(`$$${currentText}$$`);
      }
    } else {
      callback(currentText.replace(/\$\$/g, '').trim());
    }
  };

  const toggleCodeBlock = (currentText, isCode, callback) => {
    if (isCode) {
      if (!currentText.includes('```')) {
        callback("```\n" + currentText + "\n```");
      }
    ...
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4 font-sans text-slate-800 text-left">
      <div className="bg-white rounded-[3rem] w-full max-w-6xl max-h-[95vh] overflow-hidden flex flex-col shadow-2xl">
        <div className="p-6 border-b flex justify-between items-center bg-slate-50">
          <div className="flex items-center gap-3 text-slate-800"><div className="bg-indigo-600 p-2 rounded-xl text-white flex items-center justify-center"><Settings size={20}/></div><h3 className="text-xl font-black tracking-tight">考題內容配置器</h3></div>
        ...
        <div className="flex flex-col lg:flex-row overflow-hidden flex-1">
          <form onSubmit={(e) => { e.preventDefault(); saveQuestion(new FormData(e.target)); }} className="p-8 space-y-6 overflow-y-auto lg:w-1/2 border-r border-slate-100 text-left bg-white">
            <div className="space-y-2 text-left">
              <div className="flex justify-between items-end px-1">
                <label className="block text-[0.65rem] font-black text-slate-400 mb-2 uppercase tracking-widest text-left">題目敘述內容</label>
                <div className="flex gap-4">
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="checkbox" className="w-4 h-4 rounded border-slate-200 text-indigo-600 focus:ring-0" onChange={(e) => toggleFormula(localQ.text, e.target.checked, (v) => handleUpdate('text', v))} />
                    <span className="text-[0.6rem] font-bold text-slate-400 group-hover:text-indigo-600 transition uppercase">公式 ($$)</span>
                  </label>
                  <label className="flex items-center gap-1.5 cursor-pointer group">
                    <input type="checkbox" className="w-4 h-4 rounded border-slate-200 text-indigo-600 focus:ring-0" onChange={(e) => toggleCodeBlock(localQ.text, e.target.checked, (v) => handleUpdate('text', v))} />
                    <span className="text-[0.6rem] font-bold text-slate-400 group-hover:text-indigo-600 transition uppercase">程式碼 (```)</span>
                  </label>
                </div>
              </div>
              <textarea name="text" value={localQ.text} onChange={(e) => handleUpdate('text', e.target.value)} required className="w-full p-5 bg-slate-50 rounded-2xl h-40 outline-none font-mono text-sm focus:bg-white border-2 border-transparent focus:border-indigo-600 transition-all shadow-inner text-slate-700 leading-relaxed"></textarea>
            </div>
            
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-[0.65rem] font-black text-slate-400 mb-2 uppercase tracking-widest ml-1 text-left">題型模式</label>
              <select name="type" value={localQ.type} onChange={(e) => handleUpdate('type', e.target.value)} className="w-full p-4 bg-slate-50 rounded-2xl font-bold border-none outline-none shadow-inner text-slate-700 appearance-none cursor-pointer"><option value="choice">選擇題 (自動計分)</option><option value="fill">填空題 (人工閱卷)</option></select></div>
              <div><label className="block text-[0.65rem] font-black text-slate-400 mb-2 uppercase tracking-widest ml-1 text-left">配分分數</label>
              <input name="points" type="number" value={localQ.points} onChange={(e) => handleUpdate('points', e.target.value)} className="w-full p-4 bg-slate-50 rounded-2xl font-bold shadow-inner text-slate-700 text-center" /></div>
            </div>

            <div className="text-left"><label className="block text-[0.65rem] font-black text-slate-400 mb-2 uppercase tracking-widest ml-1 text-left">正確答案 (標竿參考)</label>
            ...
            {localQ.type === 'choice' && (
              <div className="space-y-3 text-left">
                <label className="block text-[0.65rem] font-black text-slate-400 uppercase tracking-widest ml-1 text-left">選項編輯</label>
                {['A', 'B', 'C', 'D'].map((lab, i) => (
                  <div key={lab} className="flex gap-2 items-start group">
                    <div className="flex flex-col items-center gap-2 mt-1">
                      <span className="w-8 h-8 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center text-xs font-black shrink-0">{lab}</span>
                      <input type="checkbox" title="插入公式" className="w-4 h-4 rounded border-slate-200 text-indigo-600 focus:ring-0 cursor-pointer" onChange={(e) => toggleFormula(localQ.options[i], e.target.checked, (v) => handleOptionUpdate(i, v))} />
                    </div>
                    <textarea name={`opt${lab}`} value={localQ.options[i]} placeholder={`輸入選項 ${lab}...`} onChange={(e) => handleOptionUpdate(i, e.target.value)} className="w-full p-3 bg-slate-50 rounded-xl border-none outline-none focus:ring-2 focus:ring-indigo-600 font-mono text-xs text-slate-700 min-h-[60px] leading-relaxed transition-colors" />
                  </div>
                ))}
              </div>
            )}
            <div className="flex gap-4 pt-4 justify-center">
              <button type="submit" className="flex-1 bg-slate-900 text-white py-5 rounded-2xl font-black shadow-lg hover:bg-black transition flex items-center justify-center text-white active:scale-95 text-lg"><Save size={20} className="mr-2"/> 儲存考題內容</button>
              <button type="button" onClick={() => setEditingQ(null)} className="flex-1 bg-slate-100 text-slate-600 py-5 rounded-2xl font-black hover:bg-slate-200 transition flex items-center justify-center text-slate-600 active:scale-95 text-lg">取消返回</button>
            </div>
          </form>

          <div className="p-8 lg:w-1/2 bg-slate-50 overflow-y-auto flex flex-col text-left">
            <div className="flex items-center justify-between mb-6">
              <h4 className="text-[0.65rem] font-black text-indigo-500 uppercase tracking-[0.2em] flex items-center gap-2"><Eye size={14}/> 考生視角即時預覽</h4>
              <div className="px-3 py-1 bg-indigo-100 text-indigo-600 text-[0.55rem] font-black rounded-lg uppercase tracking-tighter">Live Update</div>
            </div>
            <div className="space-y-6 flex-1 text-left">
              <div className="bg-white p-8 rounded-[2.5rem] shadow-xl border border-white flex flex-col text-left">
                <span className="text-[0.55rem] font-black text-slate-300 uppercase block mb-4 tracking-widest text-left">Question Block</span>
                <FormattedText content={localQ.text || "等待輸入內容..."} className="text-base text-slate-800 text-left" />
              </div>
              {localQ.type === 'choice' ? (
                <div className="space-y-3">{localQ.options.map((opt, i) => (
                  <div key={i} className="bg-white p-5 rounded-2xl border border-slate-100 flex items-start gap-4 shadow-sm text-left">
                    <div className="w-8 h-8 rounded-xl bg-slate-50 flex shrink-0 items-center justify-center text-[0.65rem] font-black text-slate-400 mt-0.5">{String.fromCharCode(65+i)}</div>
                    <FormattedText content={opt || "選項內容預覽..."} className="text-sm text-slate-600 flex-1 text-left" />
                  </div>
                ))}</div>
              ) : <div className="bg-indigo-50/50 p-10 rounded-[2.5rem] border-2 border-dashed border-indigo-100 text-indigo-300 font-black text-center italic text-sm">此處將顯示文字填空輸入框</div>}
            </div>
          </div>
        </div>
      </div>
...
const AdminDashboard = memo(({ records, exams, questions, onBack, onTestExam, appId, user, onShowSuccess }) => {
  ...
  const saveExam = async (e) => {
    ...
    try {
      ...
      setEditingExam(null);
      onShowSuccess(editingExam.id === 'new' ? "考卷建立成功！" : "修改成功！");
  ...
  const exportToExcel = useCallback(() => {
    ...
    if (filtered.length === 0) return alert("無紀錄可匯出。");
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-left pb-20 text-slate-800">
      <nav className="bg-slate-900 text-white p-4 px-8 flex justify-between items-center sticky top-0 z-40 shadow-xl">
        <div className="font-black text-xl flex items-center gap-3 text-white"><div className="bg-indigo-500 p-2 rounded-xl flex items-center justify-center text-white"><ShieldCheck size={20} /></div>助教控制中心</div>
      ...
      <div className="flex-1 max-w-6xl w-full mx-auto p-6 md:p-10 flex flex-col items-center md:items-start text-left">
        ...
        {tab === 'records' && (
          <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden text-slate-800 w-full text-left">
            <div className="p-6 border-b flex flex-col sm:flex-row justify-between items-center gap-4 bg-slate-50/50 text-left">
              <h3 className="font-black text-slate-800 text-xl text-left">全體考生測驗數據</h3>
              <div className="flex items-center gap-2 text-white text-left">
                <select className="px-4 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold outline-none text-slate-700 cursor-pointer" onChange={(e)=>setSelectedExamId(e.target.value)} value={selectedExamId || ''}><option value="">-- 選取特定考卷匯出 --</option>{exams.map(e => <option key={e.id} value={e.id}>{e.title}</option>)}</select>
        ...
        {tab === 'exams' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full text-left">
            {exams.map(e => (
              <div key={e.id} className={`bg-white p-8 rounded-[2.5rem] border-2 flex flex-col justify-between ${e.isActive ? 'border-green-500 shadow-xl ring-8 ring-green-500/5' : 'border-slate-100'} text-slate-800 text-left`}>
                <div className="text-left">
                  <div className="flex justify-between mb-4 text-left">
                    ...
                    <div className="flex gap-2 text-slate-300 text-left"><button onClick={() => setEditingExam(e)} className="hover:text-indigo-500 flex items-center justify-center"><Edit3 size={18}/></button><button onClick={async () => { if(confirm('確定刪除考卷？')) { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'exams', e.id)); onShowSuccess("考卷已移除！"); } }} className="hover:text-red-500 flex items-center justify-center"><Trash2 size={18}/></button></div>
                  </div>
                ...
                <div className="flex gap-3 pt-4 border-t border-slate-50 text-left">
                  <button onClick={async () => {
                    ...
                    onShowSuccess("測驗狀態已更新！");
                  }} className={`flex-1 py-3 rounded-2xl font-black text-xs transition flex items-center justify-center text-white ${e.isActive ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-green-600 text-white shadow-lg shadow-green-500/20'}`}>{e.isActive ? '停止測驗' : '啟動正式測驗'}</button>
        ...
        {tab === 'questions' && (
          <div className="space-y-6 w-full text-left">
            <div className="flex flex-col md:flex-row justify-between gap-4 bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-200 text-left text-slate-800">
              <div className="flex-1 text-left"><label className="block text-[0.6rem] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1 text-left">題庫定位器</label>
              <select className="w-full md:w-80 p-4 bg-slate-50 rounded-2xl outline-none font-black text-slate-700 shadow-inner border-none cursor-pointer text-left" value={selectedExamId || ''} onChange={(e) => setSelectedExamId(e.target.value)}><option value="">-- 選取特定考卷管理題目 --</option>{exams.map(e => <option key={e.id} value={e.id}>{e.title}</option>)}</select></div>
              {selectedExamId && <button onClick={() => setEditingQ({ id: 'new', type: 'choice', text: '', answer: 'A', points: 4, options: ['', '', '', ''] })} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl flex items-center justify-center gap-2 hover:bg-indigo-700 font-black transition shadow-lg active:scale-95 text-center text-white"><Plus size={20} /> 新增考題</button>}
            </div>
            <div className="grid grid-cols-1 gap-6 text-left">{selectedExamId && questions.filter(q => q.examId === selectedExamId).map((q, idx) => (
              <div key={q.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200 flex flex-col md:flex-row gap-6 items-start text-left text-slate-800">
                ...
                <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 text-left text-slate-700 leading-relaxed"><FormattedText content={q.text} className="text-sm" /></div>
                ...
                <div className="flex md:flex-col gap-2 shrink-0 text-left"><button onClick={() => setEditingQ(q)} className="p-4 bg-slate-50 text-indigo-500 hover:bg-indigo-500 hover:text-white rounded-2xl transition flex items-center justify-center shadow-sm active:scale-95 text-indigo-500"><Edit3 size={20}/></button>
                <button onClick={async () => { if(confirm('確定刪除題目？')) { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'questions', q.id)); onShowSuccess("題目已移除！"); } }} className="p-4 bg-slate-50 text-red-500 hover:bg-red-500 hover:text-white rounded-2xl transition flex items-center justify-center shadow-sm active:scale-95 text-red-500"><Trash2 size={20}/></button></div>
              </div>
            ))}</div>
          </div>
        )}
          ...
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col text-left">
            <div className="p-8 border-b flex justify-between items-center bg-slate-50 text-left">
              <div className="flex items-center gap-6 text-left">
                <div className="w-16 h-16 bg-slate-900 rounded-3xl flex flex-col items-center justify-center text-white shadow-lg text-white">
                  <span className="text-[0.5rem] font-bold opacity-50 uppercase tracking-widest text-white">Total</span>
                  <span className="text-2xl font-black tracking-tighter text-white">{(viewingRecord.choiceScore || 0) + gradingAnswers.reduce((acc, curr) => { const q = viewingRecord.answers.find(ans => ans.qId === curr.qId); return acc + (q?.type === 'fill' ? curr.val : 0); }, 0)}</span>
                </div>
                <div className="text-left text-slate-800"><h3 className="font-black text-2xl text-slate-800 text-left">{viewingRecord.studentName}</h3><p className="text-xs text-slate-400 font-bold uppercase tracking-widest text-left">{viewingRecord.studentId} • 閱卷系統</p></div>
              </div>
            ...
            <div className="p-10 space-y-8 overflow-y-auto flex-1 bg-slate-50/50 text-left text-slate-800">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">{viewingRecord.answers.map((a, idx) => (<div key={idx} className={`p-8 rounded-[2.5rem] border-2 bg-white transition-all text-left ${a.type === 'choice' ? 'border-slate-100 opacity-60' : 'border-indigo-500 shadow-xl ring-8 ring-indigo-500/5'}`}>
                ...
                <div className="space-y-4 text-left"><div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100 text-left shadow-inner"><label className="text-[0.6rem] font-black text-indigo-400 uppercase block mb-1 text-left tracking-widest">考生回答</label><FormattedText content={a.studentAns || "(未答)"} className="font-black text-indigo-900 text-lg text-left" /></div>
                <div className="p-4 bg-green-50 rounded-2xl border border-green-100 text-left shadow-inner"><label className="text-[0.6rem] font-black text-green-400 uppercase block mb-1 text-left tracking-widest">正確答案</label><FormattedText content={a.correctAns} className="font-black text-green-800 text-lg text-left" /></div>
                {a.type === 'fill' && (<div className="pt-4 flex items-center gap-4 text-left text-slate-800"><label className="text-xs font-black uppercase text-left tracking-widest">批改給分：</label><input type="number" max={a.points} min={0} className="w-24 p-3 bg-white border-2 border-indigo-200 rounded-xl font-black text-center outline-none shadow-sm text-slate-800 focus:border-indigo-600 transition-all" value={gradingAnswers.find(ga => ga.qId === a.qId)?.val ?? 0} onChange={(e) => { const val = Math.min(a.points, Math.max(0, parseInt(e.target.value) || 0)); setGradingAnswers(prev => prev.map(p => p.qId === a.qId ? { ...p, val } : p)); }} /><span className="text-slate-400 text-xs font-bold text-left text-slate-400">/ {a.points}</span></div>)}</div></div>))}</div>
            </div>
            <div className="p-8 border-t bg-white flex gap-4 justify-center shadow-inner text-left text-white"><button onClick={saveManualGrades} className="px-12 py-5 bg-indigo-600 text-white rounded-[2rem] font-black hover:bg-indigo-700 transition flex items-center justify-center gap-2 shadow-xl shadow-indigo-500/20 active:scale-95 text-white"><Save size={20}/> 儲存更新</button>
            <button onClick={() => setViewingRecord(null)} className="px-12 py-5 bg-slate-100 text-slate-600 rounded-[2rem] font-black hover:bg-slate-200 transition flex items-center justify-center text-slate-600 active:scale-95">取消</button></div>
          </div>
        </div>
      )}
      ...
      {editingExam && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4 text-slate-800 text-left">
          <div className="bg-white rounded-[2.5rem] p-10 w-full max-w-md shadow-2xl flex flex-col items-center text-left">
            <h3 className="text-xl font-black mb-8 w-full text-center text-slate-800 text-left">考卷標題設定</h3>
            <form onSubmit={saveExam} className="space-y-6 w-full text-left">
              <div className="text-left w-full text-left"><label className="text-[0.65rem] font-black text-slate-400 mb-2 block uppercase ml-1 text-left tracking-widest">考卷標題</label>
              ...
              <div className="flex gap-4 justify-center text-left text-white"><button type="submit" disabled={isSaving} className="flex-1 bg-slate-900 text-white py-4 rounded-xl font-black shadow-lg hover:bg-black transition text-white text-left flex items-center justify-center active:scale-95">{isSaving ? '儲存中...' : '確認儲存'}</button>
              <button type="button" onClick={() => setEditingExam(null)} className="flex-1 bg-slate-100 py-4 rounded-xl font-black text-slate-600 hover:bg-slate-200 transition flex items-center justify-center text-slate-600 active:scale-95">取消</button></div>
            </form>
          </div>
        </div>
      )}
...
const App = () => {
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-slate-900 font-sans"><div className="animate-spin rounded-full h-10 w-10 border-t-2 border-indigo-600"></div></div>;
