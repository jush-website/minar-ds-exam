    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-900 p-4 font-sans">
      <div className="bg-white p-12 rounded-[3rem] w-full max-w-md shadow-2xl text-center">
        ...
        <input 
          type="password" 
          autoFocus
          className="w-full p-5 bg-slate-50 rounded-2xl border-2 border-transparent focus:border-indigo-500 outline-none transition-all font-mono text-center text-2xl mb-6 shadow-inner" 
          placeholder="••••"
    ...
    <div className="min-h-screen bg-slate-50 font-sans select-none pb-20 text-left">
      <header className={`border-b sticky top-0 z-30 p-4 flex justify-between items-center px-8 shadow-sm ${isTestMode ? 'bg-amber-500 text-white' : 'bg-white text-slate-800'}`}>
        ...
        <div className="flex items-center gap-4">
          ...
          {isTestMode && (
            <button onClick={onCancel} className="bg-white/10 hover:bg-white/20 p-2 rounded-lg transition"><ArrowLeft size={18} /></button>
          )}
        </div>
      </header>
            ...
            {q.type === 'choice' ? (
              <div className="grid grid-cols-1 gap-3">
                {q.options.map((opt, i) => {
                  ...
                  return (
                    <button key={i} onClick={() => setCurrentAnswers({...currentAnswers, [q.id]: val})} className={`w-full text-left p-5 rounded-2xl border-2 transition-all flex items-center gap-5 ${active ? 'border-indigo-600 bg-indigo-50 shadow-md' : 'border-slate-50 hover:bg-slate-50 text-slate-600'}`}>
  ...
  <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4 text-center font-sans">
    <div className="bg-white p-10 rounded-[3rem] shadow-2xl w-full max-w-lg text-center border border-slate-100">
      ...
      <p className="text-slate-400 font-medium mb-12 leading-relaxed">
        {isTestMode ? "測試數據不會被紀錄，僅供顯示預覽。" : (examResult?.isTerminated ? '系統偵測到視窗切換，已自動收卷。' : '成績已成功存檔。')}
      </p>
      <div className={`bg-gradient-to-br p-10 rounded-[2.5rem] mb-10 shadow-xl text-white ${isTestMode ? 'from-amber-500 to-amber-700' : (examResult?.isTerminated ? 'from-red-500 to-red-700' : 'from-slate-800 to-slate-900')}`}>
        <span className="text-[0.7rem] font-black uppercase tracking-[0.3em] opacity-60 mb-2 block">Final Score</span>
...
const AdminDashboard = ({ records, exams, questions, onBack, onTestExam, appId }) => {
  ...
  const exportToExcel = () => {
    ...
    filteredRecords.forEach(r => {
      csvContent += `"${r.examTitle}","${r.studentId}","${r.studentName}",${r.score},"${new Date(r.timestamp).toLocaleString()}","${r.isTerminated ? "強制終止" : "正常交卷"}"\n`;
    });
  ...
  const saveExam = async (e) => {
    e.preventDefault();
    if (!auth.currentUser) return alert("請等待登入完成");
    ...
    try {
      ...
      alert("建立失敗，請檢查資料庫規則設定：" + err.message);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-left pb-20">
      ...
      <div className="flex-1 max-w-6xl w-full mx-auto p-6 md:p-10">
        <div className="flex flex-wrap gap-2 mb-10 bg-white p-2 rounded-3xl shadow-sm border border-slate-200 w-fit">
          <button onClick={() => setTab('records')} className={`flex items-center gap-2 px-8 py-3 rounded-2xl font-black transition-all ${tab === 'records' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}><ClipboardList size={18} /> 紀錄</button>
          <button onClick={() => setTab('exams')} className={`flex items-center gap-2 px-8 py-3 rounded-2xl font-black transition-all ${tab === 'exams' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}><Layers size={18} /> 考卷</button>
          <button onClick={() => setTab('questions')} className={`flex items-center gap-2 px-8 py-3 rounded-2xl font-black transition-all ${tab === 'questions' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}><Settings size={18} /> 題庫</button>
        </div>

        {tab === 'records' && (
          <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden">
            <div className="p-6 border-b flex justify-between items-center">
              <h3 className="font-black text-slate-800">測驗紀錄</h3>
              {selectedExamId && <button onClick={exportToExcel} className="bg-green-600 text-white px-5 py-2 rounded-xl text-xs font-black flex items-center gap-2 shadow-lg"><FileDown size={14}/> 匯出 Excel</button>}
            </div>
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50 border-b text-[0.65rem] font-black text-slate-400 uppercase tracking-widest">
                <tr><th className="p-6">姓名/學號</th><th>狀態</th><th>分數 (點擊修改)</th><th>時間</th><th className="p-6 text-center">操作</th></tr>
              </thead>
                ...
                {records.sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp)).map(r => (
                  <tr key={r.id} className="hover:bg-slate-50 transition">
                    <td className="p-6">
                      ...
                      <div className="font-mono text-[0.6rem] text-indigo-500 font-black">{r.studentId}</div>
                    </td>
                    <td><span className={`px-3 py-1 rounded-full text-[0.6rem] font-black uppercase ${r.isTerminated ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>{r.isTerminated ? "強制終止" : "正常"}</span></td>
                    <td>
                      {editingScoreId === r.id ? <input type="number" className="w-16 p-1 border rounded font-black text-center" defaultValue={r.score} onBlur={(e) => handleUpdateScore(r.id, e.target.value)} onKeyDown={(e) => {if(e.key==='Enter') handleUpdateScore(r.id, e.target.value);}} autoFocus /> : 
                      <button onClick={() => setEditingScoreId(r.id)} className={`px-4 py-1 rounded-full font-black flex items-center gap-2 group ${r.score>=60 ? 'text-green-600 bg-green-50' : 'text-red-600 bg-red-50'}`}>{r.score} <Edit3 size={12} className="opacity-0 group-hover:opacity-100 transition"/></button>}
                    </td>
                    <td className="text-slate-400 text-[0.65rem]">{new Date(r.timestamp).toLocaleString()}</td>
                    <td className="p-6 text-center"><button onClick={() => setViewingRecord(r)} className="bg-slate-100 text-slate-500 px-4 py-2 rounded-xl text-xs font-black">詳情</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <div key={e.id} className={`bg-white p-8 rounded-[2.5rem] border-2 flex flex-col justify-between ${e.isActive ? 'border-green-500 shadow-xl' : 'border-slate-100'}`}>
                <div>
                  <div className="flex justify-between mb-4">
                    <span className={`px-3 py-1 rounded-full text-[0.6rem] font-black uppercase ${e.isActive ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-400'}`}>{e.isActive ? '測驗中' : '已關閉'}</span>
                    <div className="flex gap-2"><button onClick={() => setEditingExam(e)} className="text-slate-300 hover:text-indigo-500 transition"><Edit3 size={18}/></button><button onClick={async () => { if(confirm('確定刪除？')) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'exams', e.id)); }} className="text-slate-300 hover:text-red-500 transition"><Trash2 size={18}/></button></div>
                  </div>
                ...
                <div className="flex gap-3 pt-4 border-t border-slate-50">
                  ...
                  }} className={`flex-1 py-3 rounded-2xl font-black text-xs transition ${e.isActive ? 'bg-slate-100 text-slate-600' : 'bg-green-600 text-white shadow-lg'}`}>{e.isActive ? '停止' : '啟用'}</button>
                  <button onClick={() => onTestExam(e)} className="px-6 py-3 rounded-2xl bg-slate-900 text-white font-black text-xs flex items-center gap-2"><PlayCircle size={16}/> 測試</button>
                </div>
              </div>
            ))}
            <button onClick={() => setEditingExam({ id: 'new', title: '' })} className="h-full min-h-[200px] border-2 border-dashed border-slate-200 rounded-[2.5rem] flex flex-col items-center justify-center gap-4 text-slate-400 hover:border-indigo-500 hover:text-indigo-500 transition group"><Plus size={40} className="group-hover:scale-110 transition"/><span className="font-black">新增考卷</span></button>
          </div>
        )}
          ...
          <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between gap-4 bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-200">
              <div className="flex-1">
                ...
                <select className="w-full md:w-80 p-4 bg-slate-50 rounded-2xl outline-none font-black text-slate-700 cursor-pointer shadow-inner" value={selectedExamId || ''} onChange={(e) => setSelectedExamId(e.target.value)}>
                  <option value="">-- 選取考卷 --</option>
                  {exams.map(e => <option key={e.id} value={e.id}>{e.title}</option>)}
                </select>
              </div>
              {selectedExamId && <button onClick={() => setEditingQ({ id: 'new', type: 'choice', text: '', answer: 'A', points: 4, options: ['', '', '', ''] })} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl flex items-center gap-2 hover:bg-indigo-700 font-black transition shadow-lg shadow-indigo-500/30"><Plus size={20} /> 新增題目</button>}
            </div>
            {selectedExamId && questions.filter(q => q.examId === selectedExamId).map((q, idx) => (
              <div key={q.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200 flex flex-col md:flex-row gap-6 items-start">
                <div className="flex-1 w-full overflow-hidden text-left">
                  <div className="flex items-center gap-3 mb-4"><span className="bg-slate-100 text-slate-400 px-3 py-1 rounded-xl text-xs font-black">第 {idx+1} 題</span><span className="text-indigo-600 font-black text-xs">{q.points} Pts</span></div>
                  <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 font-mono text-sm text-slate-700 whitespace-pre-wrap">{q.text}</div>
                  <div className="mt-4 font-black text-green-600 px-2 text-sm">解：{q.answer}</div>
                </div>
                <div className="flex md:flex-col gap-2"><button onClick={() => setEditingQ(q)} className="p-4 bg-slate-50 text-indigo-500 hover:bg-indigo-500 hover:text-white rounded-2xl transition"><Edit3 size={20}/></button><button onClick={async () => { if(confirm('刪除題目？')) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'questions', q.id)); }} className="p-4 bg-slate-50 text-red-500 hover:bg-red-500 hover:text-white rounded-2xl transition"><Trash2 size={20}/></button></div>
              </div>
            ))}
          </div>
        )}
      ...
      {editingExam && <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4 font-sans"><div className="bg-white rounded-[2.5rem] p-10 w-full max-w-md shadow-2xl"><h3 className="text-xl font-black mb-8">考卷設定</h3><form onSubmit={saveExam} className="space-y-6 text-left"><div><label className="text-[0.65rem] font-black text-slate-400 mb-2 block uppercase">標題</label><input name="title" defaultValue={editingExam.title} required className="w-full p-5 bg-slate-50 rounded-2xl font-bold shadow-inner" /></div><div className="flex gap-4"><button type="submit" className="flex-1 bg-slate-900 text-white py-4 rounded-xl font-black shadow-lg">確認</button><button type="button" onClick={() => setEditingExam(null)} className="flex-1 bg-slate-100 py-4 rounded-xl font-black">取消</button></div></form></div></div>}

      {editingQ && <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4 font-sans"><div className="bg-white rounded-[3rem] w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl"><div className="p-8 border-b font-black text-slate-800 text-xl">考題編輯</div><form onSubmit={saveQuestion} className="p-10 space-y-6 overflow-y-auto flex-1 text-left"><div><label className="text-[0.65rem] font-black text-slate-400 mb-2 block uppercase">題目內容</label><textarea name="text" defaultValue={editingQ.text} required className="w-full p-6 bg-slate-50 rounded-3xl h-64 outline-none font-mono text-sm focus:bg-white border-2 border-transparent focus:border-indigo-600 transition-all shadow-inner"></textarea></div><div className="grid grid-cols-2 gap-4"><div><label className="text-[0.65rem] font-black text-slate-400 mb-2 block uppercase">題型</label><select name="type" defaultValue={editingQ.type} className="w-full p-4 bg-slate-50 rounded-2xl font-bold border-none outline-none shadow-inner"><option value="choice">選擇題</option><option value="fill">填空題</option></select></div><div><label className="text-[0.65rem] font-black text-slate-400 mb-2 block uppercase">配分</label><input name="points" type="number" defaultValue={editingQ.points} className="w-full p-4 bg-slate-50 rounded-2xl font-bold shadow-inner" /></div></div><div><label className="text-[0.65rem] font-black text-slate-400 mb-2 block uppercase">正確答案</label><input name="answer" defaultValue={editingQ.answer} required className="w-full p-4 bg-slate-50 rounded-2xl font-mono font-black text-indigo-600 shadow-inner" /></div>{editingQ.type === 'choice' && <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-100 p-8 rounded-[2rem]">{['A', 'B', 'C', 'D'].map((lab, i) => (<div key={lab}><label className="text-[0.6rem] font-black text-slate-400 mb-1 block uppercase">選項 {lab}</label><input name={`opt${lab}`} defaultValue={editingQ.options[i]} placeholder={`內容...`} className="w-full p-3 rounded-xl border-none outline-none focus:ring-2 focus:ring-indigo-600 font-medium bg-white" /></div>))}</div>}<div className="flex gap-4 pt-4"><button type="submit" className="flex-1 bg-slate-900 text-white py-5 rounded-2xl font-black shadow-lg">儲存題目</button><button type="button" onClick={() => setEditingQ(null)} className="flex-1 bg-slate-100 text-slate-600 py-5 rounded-2xl font-black">取消</button></div></form></div></div>}

      {viewingRecord && <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-xl z-[60] flex items-center justify-center p-4 font-sans"><div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col"><div className="p-8 border-b flex justify-between items-center bg-slate-50"><div className="flex items-center gap-4"><div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center text-white font-black text-xl">{viewingRecord.score}</div><div><h3 className="font-black text-slate-800 text-xl">{viewingRecord.studentName}</h3><p className="text-xs text-slate-400 font-bold tracking-widest">{viewingRecord.studentId}</p></div></div><button onClick={() => setViewingRecord(null)} className="p-2 text-slate-300 hover:text-red-500"><X /></button></div><div className="p-10 space-y-6 overflow-y-auto flex-1">{viewingRecord.answers.map((a, idx) => (<div key={idx} className={`p-8 rounded-[2rem] border-2 ${a.isCorrect ? 'border-green-100 bg-green-50/20' : 'border-red-100 bg-red-50/20'}`}><div className="flex justify-between items-center mb-4 text-xs font-black uppercase text-slate-400"><span>題目 {idx+1}</span><span className={a.isCorrect ? 'text-green-600' : 'text-red-600'}>{a.isCorrect ? '正確' : '錯誤'}</span></div><div className="bg-white/50 p-6 rounded-2xl font-mono text-sm mb-4 border border-white/80 whitespace-pre-wrap">{a.qText}</div><div className="grid grid-cols-2 gap-4"><div className="p-4 bg-white/80 rounded-2xl text-center"><div className="text-[0.6rem] font-bold text-slate-400 mb-1">考生回答</div><div className={`font-black text-lg ${a.isCorrect ? 'text-indigo-600' : 'text-red-600'}`}>{a.studentAns || "未答"}</div></div><div className="p-4 bg-white/80 rounded-2xl text-center"><div className="text-[0.6rem] font-bold text-slate-400 mb-1">正確解答</div><div className="font-black text-green-600">{a.correctAns}</div></div></div></div>))}</div><div className="p-8 border-t text-center"><button onClick={() => setViewingRecord(null)} className="px-10 py-4 bg-slate-900 text-white rounded-2xl font-black shadow-lg">關閉視窗</button></div></div></div>}
    </div>
  );
};
