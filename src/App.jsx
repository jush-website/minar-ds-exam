...
import { 
  ...
  AlertTriangle, PlayCircle, BookOpen, Layers, X, Check, FileDown, ArrowLeft, Save, InfoIcon
} from 'lucide-react';
...
const StudentLoginView = memo(({ studentInfo, setStudentInfo, onStartExam, onBack, isChecking }) => (
  <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4 font-sans text-slate-800">
    <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl w-full max-w-md border border-slate-100 flex flex-col items-center">
      <h2 className="text-2xl font-black text-slate-800 mb-2 text-center">身份核對</h2>
      <p className="text-slate-400 text-center mb-8 text-sm italic">學號一旦提交即鎖定紀錄，不可重複考試</p>
      <div className="space-y-4 w-full text-left">
        ...
        <div className="pt-4 flex flex-col items-center gap-3 w-full">
          <button disabled={!studentInfo.id || !studentInfo.name || isChecking} onClick={onStartExam} className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black hover:bg-black disabled:opacity-30 transition-all shadow-xl flex items-center justify-center gap-2 group text-white">
            {isChecking ? "驗證中..." : "核對資料並進入"} <ArrowRight size={20} />
          </button>
  ...
  <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4 font-sans text-slate-800 text-center">
    <div className="bg-white p-10 rounded-[3rem] shadow-2xl w-full max-w-lg text-center border border-slate-100 flex flex-col items-center">
      ...
      <div className={`w-full bg-gradient-to-br p-10 rounded-[2.5rem] mb-10 shadow-xl text-white ${isTestMode ? 'from-amber-500 to-amber-700' : (examResult?.isTerminated ? 'from-red-500 to-red-700' : 'from-slate-800 to-slate-900')}`}>
        <span className="text-[0.7rem] font-black uppercase tracking-[0.3em] opacity-60 mb-2 block text-center">Choice Score (選擇得分)</span>
...
const AdminDashboard = memo(({ records, exams, questions, onBack, onTestExam, appId, user }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-left pb-20 text-slate-800">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead className="bg-slate-50 border-b text-[0.6rem] font-black text-slate-400 uppercase tracking-widest text-left">
                  <tr><th className="p-6 text-left">姓名 / 學號</th><th className="text-left">選擇分</th><th className="text-left">填空分</th><th className="text-left text-center">總分</th><th className="text-center">操作</th></tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {records.sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp)).map(r => (
                    <tr key={r.id} className="hover:bg-slate-50 transition">
                      ...
                      <td className="p-6 text-center"><button onClick={() => { setViewingRecord(r); setGradingAnswers(r.answers.map(a=>({qId:a.qId, val:a.earnedPoints}))); }} className="bg-slate-900 text-white px-5 py-2 rounded-xl text-[0.65rem] font-black hover:bg-indigo-600 transition flex items-center justify-center mx-auto">進入閱卷</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
              ...
              <div key={e.id} className={`bg-white p-8 rounded-[2.5rem] border-2 flex flex-col justify-between ${e.isActive ? 'border-green-500 shadow-xl ring-8 ring-green-500/5' : 'border-slate-100'} text-slate-800`}>
                <div>
                  <div className="flex justify-between items-start mb-4 text-left">
                    <span className={`px-3 py-1 rounded-full text-[0.6rem] font-black uppercase ${e.isActive ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-400'}`}>{e.isActive ? '開放中' : '關閉中'}</span>
                    <div className="flex gap-2 text-slate-300">
                      <button onClick={() => setEditingExam(e)} className="hover:text-indigo-500 transition-colors"><Edit3 size={18}/></button>
                      <button onClick={async () => { if(confirm('確定刪除？')) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'exams', e.id)); }} className="hover:text-red-500 transition-colors"><Trash2 size={18}/></button>
                    </div>
                  </div>
                ...
                <div className="flex gap-3 pt-4 border-t border-slate-50 text-left">
                  ...
                  }} className={`flex-1 py-3 rounded-2xl font-black text-xs transition flex items-center justify-center text-white ${e.isActive ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-green-600 text-white hover:bg-green-700 shadow-lg shadow-green-500/20'}`}>{e.isActive ? '停止測驗' : '啟動正式測驗'}</button>
          ...
          <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between gap-4 bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-200 text-left">
              <div className="flex-1 text-left">
                <label className="block text-[0.6rem] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">目標題庫</label>
                <select className="w-full md:w-80 p-4 bg-slate-50 rounded-2xl outline-none font-black text-slate-700 shadow-inner border-none cursor-pointer text-left" value={selectedExamId || ''} onChange={(e) => setSelectedExamId(e.target.value)}>
                  <option value="">-- 選取考卷管理題目 --</option>
                  {exams.map(e => <option key={e.id} value={e.id}>{e.title}</option>)}
                </select>
              </div>
              {selectedExamId && (
                <button onClick={() => setEditingQ({ id: 'new', type: 'choice', text: '', answer: 'A', points: 4, options: ['', '', '', ''] })} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl flex items-center justify-center gap-2 hover:bg-indigo-700 font-black transition shadow-lg text-white active:scale-95 text-center"><Plus size={20} /> 新增題目</button>
              )}
            </div>
                ...
                <div key={q.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200 flex flex-col md:flex-row gap-6 items-start text-left text-slate-800">
                  <div className="flex-1 w-full overflow-hidden text-left">
                    ...
                    <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 text-left">
                      <FormattedText content={q.text} className="text-sm text-slate-700" />
                    </div>
                    {q.type === 'choice' && (
                      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2 text-left">
                        {q.options.map((opt, i) => (
                          <div key={i} className="flex items-start gap-2 bg-slate-50/50 p-2 rounded-lg text-xs text-left">
                            <span className="font-black text-slate-400 text-left">{String.fromCharCode(65+i)}.</span>
                            <FormattedText content={opt} className="text-slate-600 text-left flex-1" />
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div className="flex md:flex-col gap-2 shrink-0">
                    <button onClick={() => setEditingQ(q)} className="p-4 bg-slate-50 text-indigo-500 hover:bg-indigo-500 hover:text-white rounded-2xl transition flex items-center justify-center shadow-sm text-indigo-500"><Edit3 size={20}/></button>
                    <button onClick={async () => { if(confirm('確定刪除？')) await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'questions', q.id)); }} className="p-4 bg-slate-50 text-red-500 hover:bg-red-500 hover:text-white rounded-2xl transition flex items-center justify-center shadow-sm text-red-500"><Trash2 size={20}/></button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {viewingRecord && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-[60] flex items-center justify-center p-4 text-slate-800">
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="p-8 border-b flex justify-between items-center bg-slate-50">
              <div className="flex items-center gap-6">
                ...
                <div className="text-left text-slate-800">
                  <h3 className="font-black text-2xl text-slate-800">{viewingRecord.studentName}</h3>
                  <p className="text-xs text-slate-400 font-bold uppercase tracking-widest">{viewingRecord.studentId} • 閱卷</p>
                </div>
              </div>
              <button onClick={() => setViewingRecord(null)} className="p-2 text-slate-300 hover:text-red-500 transition"><X size={32}/></button>
            </div>
                ...
                {viewingRecord.answers.map((a, idx) => (
                  <div key={idx} className={`p-8 rounded-[2.5rem] border-2 bg-white transition-all ${a.type === 'choice' ? 'border-slate-100 opacity-60' : 'border-indigo-500 shadow-xl ring-8 ring-indigo-500/5'}`}>
                    <div className="flex justify-between items-center mb-6">
                      <div className="flex items-center gap-2"><span className="bg-slate-100 text-slate-400 px-3 py-1 rounded-xl text-[0.6rem] font-black uppercase tracking-tighter">Q{idx+1}</span><span className={`px-3 py-1 rounded-xl text-[0.6rem] font-black uppercase ${a.type === 'choice' ? 'bg-blue-50 text-blue-500' : 'bg-purple-50 text-purple-500'}`}>{a.type === 'choice' ? '選擇' : '填空'}</span></div>
                      <div className="font-black text-xs text-slate-400">{a.points} Pts</div>
                    </div>
                    <div className="bg-slate-50 p-4 rounded-2xl mb-6 border border-slate-100"><FormattedText content={a.qText} className="text-xs text-slate-600 h-24 overflow-y-auto" /></div>
                    <div className="space-y-4">
                      <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100"><label className="text-[0.6rem] font-black text-indigo-400 uppercase block mb-1 text-indigo-400">考生回答</label><FormattedText content={a.studentAns || "(未答)"} className="font-black text-indigo-900 text-lg" /></div>
                      <div className="p-4 bg-green-50 rounded-2xl border border-green-100"><label className="text-[0.6rem] font-black text-green-400 uppercase block mb-1 text-green-400">正確答案</label><FormattedText content={a.correctAns} className="font-black text-green-800 text-lg" /></div>
                      {a.type === 'fill' && (
                        <div className="pt-4 flex items-center gap-4 text-slate-800"><label className="text-xs font-black uppercase">評定分數：</label>
            ...
            <div className="p-8 border-t bg-white flex gap-4 justify-center shadow-inner">
              <button onClick={saveManualGrades} className="px-12 py-5 bg-indigo-600 text-white rounded-[2rem] font-black hover:bg-indigo-700 transition flex items-center justify-center gap-2 shadow-xl text-white"><Save size={20}/> 儲存並更新分數</button>
              <button onClick={() => setViewingRecord(null)} className="px-12 py-5 bg-slate-100 text-slate-600 rounded-[2rem] font-black hover:bg-slate-200 transition flex items-center justify-center text-slate-600">取消返回</button>
            </div>
          </div>
        </div>
      )}
      ...
      {editingExam && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] p-10 w-full max-w-md shadow-2xl flex flex-col items-center">
            <h3 className="text-xl font-black mb-8 w-full text-center text-slate-800">考卷標題設定</h3>
            <form onSubmit={saveExam} className="space-y-6 w-full text-left text-slate-800">
              <div className="text-left w-full"><label className="text-[0.65rem] font-black text-slate-400 mb-2 block uppercase ml-1">標題名稱</label>
              <input name="title" defaultValue={editingExam.title} required autoFocus className="w-full p-5 bg-slate-50 rounded-2xl font-bold shadow-inner border-none outline-none focus:ring-2 focus:ring-indigo-600 transition-all text-slate-700" /></div>
              <div className="flex gap-4 justify-center w-full"><button type="submit" disabled={isSaving} className="flex-1 bg-slate-900 text-white py-4 rounded-xl font-black shadow-lg hover:bg-black transition text-white">{isSaving ? '儲存中...' : '確認'}</button><button type="button" onClick={() => setEditingExam(null)} className="flex-1 bg-slate-100 py-4 rounded-xl font-black text-slate-600 hover:bg-slate-200 text-slate-600">取消</button></div>
            </form>
          </div>
        </div>
      )}
    ...
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4 font-sans text-slate-800 text-left">
      <div className="bg-white rounded-[3rem] w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
        <div className="p-6 border-b flex justify-between items-center bg-slate-50">
          <div className="flex items-center gap-3 text-slate-800"><div className="bg-indigo-600 p-2 rounded-xl text-white"><Settings size={20}/></div><h3 className="text-xl font-black">考題內容與格式編輯</h3></div>
          <div className="hidden sm:flex items-center gap-2 text-slate-400 text-[0.6rem] font-bold uppercase tracking-widest"><InfoIcon size={14}/> 支援 LaTeX：$...$ | 程式碼：空格換行</div>
        ...
        <div className="flex flex-col lg:flex-row overflow-hidden flex-1 text-left">
          <form 
            onSubmit={(e) => {
              e.preventDefault();
              saveQuestion(new FormData(e.target));
            }} 
            className="p-8 space-y-6 overflow-y-auto lg:w-1/2 border-r border-slate-100 text-left"
          >
            <div className="text-left w-full">
              <label className="block text-[0.65rem] font-black text-slate-400 mb-2 uppercase tracking-widest ml-1">題目描述</label>
              <textarea name="text" value={localQ.text} onChange={(e) => handleUpdate('text', e.target.value)} required className="w-full p-5 bg-slate-50 rounded-2xl h-48 outline-none font-mono text-sm focus:bg-white border-2 border-transparent focus:border-indigo-600 transition-all shadow-inner text-slate-700 leading-relaxed text-left"></textarea>
            </div>
            
            <div className="grid grid-cols-2 gap-4 text-left">
              <div className="text-left"><label className="block text-[0.65rem] font-black text-slate-400 mb-2 uppercase ml-1">題型</label>
                <select name="type" value={localQ.type} onChange={(e) => handleUpdate('type', e.target.value)} className="w-full p-4 bg-slate-50 rounded-2xl font-bold border-none outline-none shadow-inner text-slate-700 cursor-pointer appearance-none">
                  <option value="choice">選擇題 (自動)</option>
                  <option value="fill">填空題 (人工)</option>
                </select>
              </div>
              <div className="text-left"><label className="block text-[0.65rem] font-black text-slate-400 mb-2 uppercase ml-1">配分</label>
                <input name="points" type="number" value={localQ.points} onChange={(e) => handleUpdate('points', e.target.value)} className="w-full p-4 bg-slate-50 rounded-2xl font-bold shadow-inner text-slate-700" />
              </div>
            </div>

            <div className="text-left w-full"><label className="block text-[0.65rem] font-black text-slate-400 mb-2 uppercase ml-1">參考解答</label>
              <textarea name="answer" value={localQ.answer} onChange={(e) => handleUpdate('answer', e.target.value)} required className="w-full p-4 bg-slate-50 rounded-2xl font-mono font-black text-indigo-600 shadow-inner min-h-[60px]"></textarea>
            </div>
            ...
            {localQ.type === 'choice' && (
              <div className="space-y-3 text-left">
                ...
                {['A', 'B', 'C', 'D'].map((lab, i) => (
                  <div key={lab} className="flex gap-2 items-start text-left">
                    ...
                    <textarea name={`opt${lab}`} value={localQ.options[i]} placeholder={`輸入內容...`} onChange={(e) => handleOptionUpdate(i, e.target.value)} className="w-full p-3 bg-slate-50 rounded-xl border-none outline-none focus:ring-2 focus:ring-indigo-600 font-mono text-xs text-slate-700 min-h-[60px] text-left leading-relaxed" />
                  </div>
                ))}
              </div>
            )}

            <div className="flex gap-4 pt-4 text-center justify-center w-full">
              ...
              <button type="button" onClick={() => setEditingQ(null)} className="flex-1 bg-slate-100 text-slate-600 py-5 rounded-2xl font-black hover:bg-slate-200 text-slate-600">取消</button>
            </div>
          </form>

          <div className="p-8 lg:w-1/2 bg-slate-50/50 overflow-y-auto text-left flex flex-col">
            ...
            <div className="space-y-6 flex-1">
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 text-left"><FormattedText content={localQ.text || "等待輸入..."} className="text-base text-slate-800 text-left" /></div>
              {localQ.type === 'choice' ? (
                <div className="space-y-3">{localQ.options.map((opt, i) => (
                  <div key={i} className="bg-white p-4 rounded-2xl border border-slate-100 flex items-start gap-4 text-left"><div className="w-6 h-6 rounded-lg bg-slate-50 flex shrink-0 items-center justify-center text-[0.6rem] font-black text-slate-400 mt-1">{String.fromCharCode(65+i)}</div><FormattedText content={opt || "選項內容..."} className="text-sm text-slate-600 flex-1 text-left" /></div>
                ))}</div>
              ) : <div className="bg-indigo-50/50 p-6 rounded-3xl border-2 border-dashed border-indigo-100 text-indigo-300 font-black text-center italic text-sm">將顯示填空輸入框</div>}
            </div>
          </div>
        </div>
      </div>
