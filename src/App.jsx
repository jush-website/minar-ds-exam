...
// --- 1. 格式化渲染組件 (支援 LaTeX, 程式碼, 以及 Base64 圖片) ---
    ...
    <div className="bg-white/5 p-10 rounded-[3rem] backdrop-blur-xl border border-white/10 flex flex-col items-center shadow-2xl max-w-sm w-full">
      ...
      <div className="grid grid-cols-1 gap-4 w-full">
        ...
        <button 
          ...
          className="bg-white/10 text-white p-5 rounded-3xl border border-white/10 hover:bg-white/20 transition-all flex flex-col items-center justify-center gap-2 active:scale-95 text-white"
        >
          <ShieldCheck size={24} />
        ...
        {questions.length === 0 ? <div className="bg-white p-20 rounded-3xl text-center text-slate-300 font-bold">目前無題目</div> : questions.map((q, idx) => (
          <div key={q.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden text-left text-slate-800">
            ...
            {(q.type === 'choice' || q.type === 'multiple') ? (
              ...
              <input type="text" className="w-full p-5 rounded-2xl bg-slate-50 border-2 border-transparent focus:border-indigo-600 focus:bg-white outline-none transition-all font-bold text-slate-800 text-left" placeholder="在此輸入您的答案..." value={currentAnswers[q.id] || ''} onChange={(e) => setCurrentAnswers(prev => ({...prev, [q.id]: e.target.value}))} />
            )}
          </div>
        ))}
  ...
  <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4 font-sans text-slate-800 text-center text-slate-800">
    <div className="bg-white p-10 rounded-[3rem] shadow-2xl w-full max-lg text-center border border-slate-100 flex flex-col items-center">
      ...
      <div className={`w-full bg-gradient-to-br p-10 rounded-[2.5rem] mb-10 shadow-xl text-white ${isTestMode ? 'from-amber-500 to-amber-700' : (examResult?.isTerminated ? 'from-red-500 to-red-700' : 'from-slate-800 to-slate-900')}`}>
        <span className="text-[0.7rem] font-black uppercase tracking-[0.3em] opacity-60 mb-2 block text-center text-white/80 text-white">Choice Score (選擇得分)</span>
        <div className="text-8xl font-black mb-4 tracking-tighter text-white text-center">{examResult?.choiceScore ?? examResult?.score}</div>
...
const AdminEditQuestionModal = memo(({ editingQ, setEditingQ, saveQuestion, showToast }) => {
  ...
  const EditorToolbar = ({ targetRef, field, optionIdx }) => (
    <div className="flex flex-wrap gap-2 mb-2">
      ...
      <div className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-50 text-emerald-600 rounded-lg text-[0.65rem] font-black border border-emerald-100 cursor-default"><ImageIcon size={14} /> 貼上圖片 (Base64)</div>
    </div>
  );
    ...
    <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4 font-sans text-slate-800 text-left">
      <div className="bg-white rounded-[3rem] w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl border border-white/20 text-left">
        <div className="p-6 border-b flex justify-between items-center bg-slate-50">
          <div className="flex items-center gap-3 text-slate-800"><div className="bg-indigo-600 p-2 rounded-xl text-white text-white text-center"><Settings size={20}/></div><h3 className="text-xl font-black">考題編輯</h3></div>
        ...
        <div className="flex flex-col lg:flex-row overflow-hidden flex-1">
          <form onSubmit={(e) => { e.preventDefault(); saveQuestion(new FormData(e.target)); }} className="p-8 space-y-6 overflow-y-auto lg:w-1/2 border-r border-slate-100 text-left">
              ...
              <div className="space-y-4">
                {['A', 'B', 'C', 'D'].map((lab, i) => (
                  <div key={lab} className="flex gap-2 items-start text-slate-800 text-left">
                    <span className="w-8 h-8 rounded-xl bg-slate-100 flex items-center justify-center text-xs font-black shrink-0 mt-1 text-slate-400 text-slate-400 text-center">{lab}</span>
            ...
            <div className="flex gap-4 pt-4 text-white">
              ...
              <button type="button" onClick={() => setEditingQ(null)} className="flex-1 bg-slate-100 text-slate-600 py-5 rounded-2xl font-black hover:bg-slate-200 text-slate-600 text-slate-600 text-center">取消</button>
            </div>
          </form>
          <div className="p-8 lg:w-1/2 bg-slate-50/50 overflow-y-auto flex flex-col text-left">
            ...
            <div className="space-y-6 flex-1 text-slate-800 text-left">
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 text-slate-800"><FormattedText content={localQ.text || "等待輸入..."} className="text-base" /></div>
              {(localQ.type === 'choice' || localQ.type === 'multiple') && (
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-800 text-left">
      <nav className="bg-slate-900 text-white p-4 px-8 flex justify-between items-center sticky top-0 z-40 shadow-xl">
        <div className="font-black text-xl flex items-center gap-3 text-white"><div className="bg-indigo-500 p-2 rounded-xl text-white text-white text-center"><ShieldCheck size={20} /></div>助教控制中心</div>
        <button onClick={onBack} className="p-2 hover:bg-white/10 rounded-xl transition text-white text-white text-center"><LogOut /></button>
      </nav>

      <div className="flex-1 max-w-6xl w-full mx-auto p-6 md:p-10 flex flex-col text-slate-800">
        ...
        {tab === 'records' && (
          <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden text-left">
            <div className="p-6 border-b flex justify-between items-center bg-slate-50/50">
              <h3 className="font-black text-xl text-slate-800">考生測驗數據</h3>
            ...
            <div className="overflow-x-auto text-left"><table className="w-full text-left border-collapse"><thead className="bg-slate-50 text-[0.6rem] font-black text-slate-400 uppercase tracking-widest text-left"><tr><th className="p-6">姓名 / 學號</th><th>選擇分</th><th>填空分</th><th className="text-center">總分</th><th className="text-center">操作</th></tr></thead>
              <tbody className="divide-y divide-slate-100 text-left text-slate-800">{records.sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp)).map(r => (
                <tr key={r.id} className="hover:bg-slate-50 transition text-slate-800"><td className="p-6 font-black">{r.studentName}<div className="font-mono text-[0.6rem] text-indigo-500">{r.studentId}</div></td>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left text-slate-800">
            {exams.map(e => (
              <div key={e.id} className={`bg-white p-8 rounded-[2.5rem] border-2 flex flex-col justify-between ${e.isActive ? 'border-green-500 shadow-xl' : 'border-slate-100'} text-left`}>
                <div className="text-left"><div className="flex justify-between mb-4 text-left"><span className={`px-3 py-1 rounded-full text-[0.6rem] font-black ${e.isActive ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-400'} text-left text-center`}>{e.isActive ? '開放中' : '關閉中'}</span>
                ...
                <h4 className="text-2xl font-black mb-6 text-slate-800">{e.title}</h4></div>
        ...
        {tab === 'questions' && (
          <div className="space-y-6 text-left">
            <div className="flex flex-col md:flex-row justify-between gap-4 bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-200 text-left">
              <select className="w-full md:w-80 p-4 bg-slate-50 rounded-2xl outline-none font-black text-slate-700 text-left" value={selectedExamId || ''} onChange={(e) => setSelectedExamId(e.target.value)}><option value="">-- 選取考卷 --</option>{exams.map(e => <option key={e.id} value={e.id}>{e.title}</option>)}</select>
              {selectedExamId && <button onClick={() => setEditingQ({ id: 'new', type: 'choice', text: '', answer: 'A', points: 4, options: ['', '', '', ''] })} className="bg-indigo-600 text-white px-8 py-4 rounded-2xl flex items-center gap-2 font-black transition text-white text-white text-center">新增題目</button>}
            </div>
            <div className="grid grid-cols-1 gap-6 text-left text-slate-800">{selectedExamId && questions.filter(q => q.examId === selectedExamId).map((q, idx) => (
              <div key={q.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200 flex flex-col md:flex-row gap-6 items-start text-left text-slate-800">
                <div className="flex-1 w-full text-left text-slate-800"><div className="flex items-center gap-3 mb-4 text-left"><span className="bg-slate-100 text-slate-400 px-3 py-1 rounded-xl text-xs font-black text-slate-400 text-center text-center">Q {idx+1}</span><span className="text-indigo-600 font-black text-xs">{q.points} Pts</span></div>
                ...
                <div className="mt-4 font-black text-green-600 px-2 text-sm text-left">答案：{q.answer}</div></div>
                <div className="flex md:flex-col gap-2 shrink-0 text-left text-slate-800 text-center"><button onClick={() => setEditingQ(q)} className="p-4 bg-slate-50 text-indigo-500 rounded-2xl shadow-sm text-indigo-500 text-center"><Edit3 size={20}/></button>
        ...
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col text-slate-800">
            <div className="p-8 border-b flex justify-between items-center bg-slate-50 text-slate-800">
              <div className="flex items-center gap-6 text-slate-800 text-left"><div className="w-16 h-16 bg-slate-900 rounded-3xl flex flex-col items-center justify-center text-white shadow-lg"><span className="text-[0.5rem] font-bold opacity-50 uppercase text-white">Score</span><span className="text-2xl font-black text-white">{(viewingRecord.choiceScore || 0) + gradingAnswers.reduce((acc, curr) => acc + curr.val, 0)}</span></div>
              <div className="text-left text-slate-800"><h3 className="font-black text-2xl text-slate-800">{viewingRecord.studentName}</h3><p className="text-xs text-slate-400 font-bold uppercase tracking-widest text-slate-400">{viewingRecord.studentId}</p></div></div>
            ...
            <div className="p-10 space-y-8 overflow-y-auto flex-1 bg-slate-50/50 text-left text-slate-800">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left text-slate-800">{viewingRecord.answers.map((a, idx) => (<div key={idx} className={`p-8 rounded-[2.5rem] border-2 bg-white ${a.type === 'fill' ? 'border-indigo-500 ring-8 ring-indigo-500/5' : 'border-slate-100 opacity-60'} text-left text-slate-800`}>
                <div className="flex justify-between items-center mb-6 text-xs font-black text-slate-400 uppercase tracking-tighter text-slate-400"><span>Q{idx+1} • {a.type === 'choice' ? '單選' : a.type === 'multiple' ? '複選' : '填空'}</span><span>{a.points} Pts</span></div>
            ...
            <div className="p-8 border-t bg-white flex gap-4 justify-center text-white text-white"><button onClick={saveManualGrades} className="px-12 py-5 bg-indigo-600 rounded-[2rem] font-black shadow-xl text-white text-white text-center">儲存分數</button><button onClick={() => setViewingRecord(null)} className="px-12 py-5 bg-slate-100 text-slate-600 rounded-[2rem] font-black text-slate-600 text-center">取消</button></div>
          </div>
        </div>
      )}
...
const App = () => {
  ...
  if (dbError) return <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-red-50 text-red-600 font-sans">
    <AlertTriangle size={48} className="mb-4 text-red-600" />
    <h1 className="text-xl font-black mb-2 text-red-600 text-center text-red-600">連線異常</h1>
    <p className="text-sm opacity-70 text-center text-red-600">系統已嘗試修復驗證錯誤。若持續出現此畫面，請重新整理頁面或檢查 Firebase 規則。</p>
    <code className="mt-4 p-4 bg-white rounded-xl text-[0.6rem] border border-red-100 max-w-md break-all text-slate-600 text-center">{dbError}</code>
  </div>;
