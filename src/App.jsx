    ...
    <div className="min-h-screen bg-slate-50 select-none pb-20 text-left font-sans">
      ...
      <main className="max-w-3xl mx-auto p-4 py-8 space-y-8 text-left">
        ...
        {questions.length === 0 ? <div className="bg-white p-20 rounded-3xl text-center text-slate-300 font-bold">目前無題目</div> : questions.map((q, idx) => (
          <div key={q.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden text-left text-slate-800">
  ...
  <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4 font-sans text-slate-800 text-center">
    <div className="bg-white p-10 rounded-[3rem] shadow-2xl w-full max-w-lg text-center border border-slate-100 flex flex-col items-center">
      ...
      <div className={`w-full bg-gradient-to-br p-10 rounded-[2.5rem] mb-10 shadow-xl text-white ${isTestMode ? 'from-amber-500 to-amber-700' : (examResult?.isTerminated ? 'from-red-500 to-red-700' : 'from-slate-800 to-slate-900')}`}>
        <span className="text-[0.7rem] font-black uppercase tracking-[0.3em] opacity-60 mb-2 block text-center text-white/80 tracking-widest">Choice Score (選擇得分)</span>
...
const AdminLoginView = memo(({ onBack, onLogin }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-900 p-4 font-sans text-slate-800 text-center">
      <div className="bg-white p-12 rounded-[3rem] w-full max-w-md shadow-2xl border border-white/10 flex flex-col items-center">
          ...
          <form onSubmit={(e) => { e.preventDefault(); saveQuestion(new FormData(e.target)); }} className="p-8 space-y-6 overflow-y-auto lg:w-1/2 border-r border-slate-100 text-left bg-white">
            <div className="space-y-2">
              <div className="flex justify-between items-end px-1">
                <label className="block text-[0.65rem] font-black text-slate-400 uppercase tracking-widest">題目敘述內容</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-[0.65rem] font-black text-slate-400 mb-2 uppercase tracking-widest ml-1">題型模式</label>
              ...
              <div><label className="block text-[0.65rem] font-black text-slate-400 mb-2 uppercase tracking-widest ml-1">配分分數</label>
            ...
            <div><label className="block text-[0.65rem] font-black text-slate-400 mb-2 uppercase tracking-widest ml-1">正確答案 (標竿參考)</label>
            ...
            {localQ.type === 'choice' && (
              <div className="space-y-3">
                <label className="block text-[0.65rem] font-black text-slate-400 uppercase tracking-widest ml-1">選項編輯 (勾選自動插入公式標籤)</label>
                {['A', 'B', 'C', 'D'].map((lab, i) => (
...
const AdminDashboard = memo(({ records, exams, questions, onBack, onTestExam, appId, user, onShowSuccess }) => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-left pb-20 text-slate-800">
      ...
      <div className="flex-1 max-w-6xl w-full mx-auto p-6 md:p-10 flex flex-col items-center md:items-start">
          ...
          <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden text-slate-800 w-full text-left">
            ...
            <div className="overflow-x-auto text-left"><table className="w-full text-left border-collapse"><thead className="bg-slate-50 border-b text-[0.6rem] font-black text-slate-400 uppercase tracking-widest text-left"><tr><th className="p-6 text-left">姓名 / 學號</th><th className="text-left">選擇分</th><th className="text-left">填空分</th><th className="text-center">總分</th><th className="text-center">操作</th></tr></thead>
              <tbody className="divide-y divide-slate-100 text-left">{records.sort((a,b)=>new Date(b.timestamp)-new Date(a.timestamp)).map(r => (
                ...
                <td className="font-bold text-slate-500 text-left">{r.choiceScore ?? r.score}</td><td className="font-bold text-indigo-600 text-left">{r.fillScore ?? 0}</td><td className="text-center"><span className={`px-4 py-1 rounded-full font-black ${ (r.totalScore ?? r.score) >= 60 ? 'text-green-600 bg-green-50' : 'text-red-600 bg-red-50' }`}>{r.totalScore ?? r.score}</span></td>
                <td className="p-6 text-center"><button onClick={() => { setViewingRecord(r); setGradingAnswers(r.answers.map(a=>({qId:a.qId, val:a.earnedPoints}))); }} className="bg-slate-900 text-white px-5 py-2 rounded-xl text-[0.65rem] font-black hover:bg-indigo-600 transition mx-auto text-white flex items-center justify-center active:scale-95">閱卷</button></td></tr>
              ))}</tbody></table></div>
          </div>
        )}
            ...
            {exams.map(e => (
              <div key={e.id} className={`bg-white p-8 rounded-[2.5rem] border-2 flex flex-col justify-between ${e.isActive ? 'border-green-500 shadow-xl ring-8 ring-green-500/5' : 'border-slate-100'} text-slate-800 text-left`}>
                ...
                <div className="flex gap-3 pt-4 border-t border-slate-50 text-left">
                  ...
                  }} className={`flex-1 py-3 rounded-2xl font-black text-xs transition flex items-center justify-center text-white ${e.isActive ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-green-600 text-white shadow-lg shadow-green-500/20'} text-slate-700`}>{e.isActive ? '停止考試' : '啟動正式測驗'}</button>
        ...
        {tab === 'questions' && (
          <div className="space-y-6 w-full text-left">
            <div className="flex flex-col md:flex-row justify-between gap-4 bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-200 text-left">
              <div className="flex-1 text-left"><label className="block text-[0.6rem] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">題庫定位器</label>
            ...
            <div className="grid grid-cols-1 gap-6 text-left">{selectedExamId && questions.filter(q => q.examId === selectedExamId).map((q, idx) => (
              <div key={q.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200 flex flex-col md:flex-row gap-6 items-start text-left">
                ...
                <div className="flex md:flex-col gap-2 shrink-0 text-left"><button onClick={() => setEditingQ(q)} className="p-4 bg-slate-50 text-indigo-500 hover:bg-indigo-500 hover:text-white rounded-2xl transition flex items-center justify-center shadow-sm active:scale-95"><Edit3 size={20}/></button>
                <button onClick={async () => { if(confirm('確定刪除題目？')) { await deleteDoc(doc(db, 'artifacts', appId, 'public', 'data', 'questions', q.id)); onShowSuccess("題目已移除！"); } }} className="p-4 bg-slate-50 text-red-500 hover:bg-red-500 hover:text-white rounded-2xl transition flex items-center justify-center shadow-sm active:scale-95"><Trash2 size={20}/></button></div>
              </div>
            ))}</div>
          </div>
        )}
      ...
      {viewingRecord && (
        <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-xl z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-[3rem] shadow-2xl w-full max-w-5xl max-h-[95vh] overflow-hidden flex flex-col text-left">
            ...
            <div className="p-10 space-y-8 overflow-y-auto flex-1 bg-slate-50/50 text-left text-slate-800">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">{viewingRecord.answers.map((a, idx) => (<div key={idx} className={`p-8 rounded-[2.5rem] border-2 bg-white transition-all text-left ${a.type === 'choice' ? 'border-slate-100 opacity-60' : 'border-indigo-500 shadow-xl ring-8 ring-indigo-500/5'}`}>
                ...
                <div className="p-4 bg-green-50 rounded-2xl border border-green-100 text-left shadow-inner"><label className="text-[0.6rem] font-black text-green-400 uppercase block mb-1 text-left tracking-widest">參考答案</label><FormattedText content={a.correctAns} className="font-black text-green-800 text-lg text-left" /></div>
                {a.type === 'fill' && (<div className="pt-4 flex items-center gap-4 text-left text-slate-800"><label className="text-xs font-black uppercase text-left tracking-widest">批改給分：</label><input type="number" max={a.points} min={0} className="w-24 p-3 bg-white border-2 border-indigo-200 rounded-xl font-black text-center outline-none shadow-sm text-slate-800 focus:border-indigo-600 transition-all" value={gradingAnswers.find(ga => ga.qId === a.qId)?.val ?? 0} onChange={(e) => { const val = Math.min(a.points, Math.max(0, parseInt(e.target.value) || 0)); setGradingAnswers(prev => prev.map(p => p.qId === a.qId ? { ...p, val } : p)); }} /><span className="text-slate-400 text-xs font-bold text-left">/ {a.points}</span></div>)}</div></div>))}</div>
            </div>
            <div className="p-8 border-t bg-white flex gap-4 justify-center shadow-inner text-left"><button onClick={saveManualGrades} className="px-12 py-5 bg-indigo-600 text-white rounded-[2rem] font-black hover:bg-indigo-700 transition flex items-center justify-center gap-2 shadow-xl shadow-indigo-500/20 active:scale-95 text-white"><Save size={20}/> 儲存更新</button>
      ...
      {editingExam && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[2.5rem] p-10 w-full max-w-md shadow-2xl flex flex-col items-center text-left">
            <h3 className="text-xl font-black mb-8 w-full text-center text-slate-800">考卷細節設定</h3>
            <form onSubmit={saveExam} className="space-y-6 w-full text-left">
              <div className="text-left w-full"><label className="text-[0.65rem] font-black text-slate-400 mb-2 block uppercase ml-1 text-left tracking-widest">考卷標題</label>
              ...
              <div className="flex gap-4 justify-center text-left"><button type="submit" disabled={isSaving} className="flex-1 bg-slate-900 text-white py-4 rounded-xl font-black shadow-lg hover:bg-black transition text-white text-left flex items-center justify-center active:scale-95">{isSaving ? '儲存中...' : '確認儲存'}</button>
...
const App = () => {
  ...
  const handleSubmit = useCallback(async (isForced = false) => {
    ...
    try {
      ...
      setExamResult(record); setView('result');
